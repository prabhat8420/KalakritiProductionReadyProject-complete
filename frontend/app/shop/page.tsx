...
export default async function ShopPage() {
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5] py-8 sm:py-10 px-4 max-w-7xl mx-auto">
      <div className="mb-8">
        <span className="text-xs font-semibold uppercase text-orange-800">Curated Marketplace</span>
        <h1 className="text-2xl sm:text-3xl font-serif font-bold text-stone-900 mt-1">Authentic Indian Handicrafts</h1>
        <p className="text-xs sm:text-sm text-stone-600">Direct from verified master artisans across India with digital authenticity certificates.</p>
      </div>
      ...
      {products.length === 0 ? (
        <div className="bg-white border border-stone-200 rounded-2xl p-12 text-center shadow-sm max-w-lg mx-auto space-y-4">
          ...
          <div>
            <h3 className="text-lg font-serif font-bold text-stone-900 mb-1">No Published Crafts Found</h3>
          ...
          {products.map((p: any) => (
            ...
            >
              <div className="relative overflow-hidden">
                ...
                />
                <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-[10px] font-bold px-2 py-0.5 rounded-full text-stone-800 shadow-2xs">
                  GI Certified
                </span>
              </div>
