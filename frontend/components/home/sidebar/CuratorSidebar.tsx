...
export default function CuratorSidebar({
  ...
  return (
    /*
     * AnimatePresence keeps the sidebar in the DOM during the exit animation.
     * Without it, `if (!isOpen) return null` unmounts instantly — no slide-out.
     * The `key` on motion elements is not needed here since we control presence
     * via the `isOpen` boolean fed to AnimatePresence's children.
     */
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex">
          ...
          <motion.div
            ...
            className="fixed inset-0 bg-[#141312]/60 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.aside
            key="sidebar-drawer"
            className="relative z-10 w-full max-w-[340px] sm:max-w-[400px] h-full bg-[#FAF6EE] border-r border-[#E3DACB] shadow-2xl flex flex-col overflow-hidden"
            role="dialog"
            aria-modal="true"
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-1.5">
                  {[
                    { href: '/shop',             icon: '🛍️', label: 'Living Craft Catalog' },
                    { href: '/traditions',        icon: '📜', label: 'Heritage Traditions' },
                    { href: '/craft-doctor',      icon: '🩺', label: 'Craft Doctor (AI Repair)', accent: true },
                    { href: '/artisan/dashboard', icon: '🎨', label: 'Master Artisan Studio' },
                  ].map(({ href, icon, label, accent }) => (
                    <Link
                      key={href}
                      href={href}
                      onClick={onClose}
                      className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl border text-xs font-mono font-semibold transition shadow-xs group ${
                        accent
                          ? 'bg-[#0F766E]/10 border-[#0F766E]/30 text-[#0F766E] hover:bg-[#0F766E]/20 font-bold'
                          : 'bg-white border-[#E3DACB] text-[#1C1917] hover:border-[#8C3826] hover:bg-[#F7F2E7]'
                      }`}
                    >
                      <div className="flex items-center gap-2.5">
                      ...
                      <span className={accent ? 'text-xs' : 'text-[#8C3826] text-xs'}>→</span>
                    </Link>
                  ))}
                </div>
              </div>
